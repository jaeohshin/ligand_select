...
Usage: python ligand_select/steps/step3_cluster_poses.py --config targets/T2383/config.yaml
"""
...
def process_pocket(pocket: str,
    ...
    for i, row in members.iterrows():
        coords = load_aligned_ligand(Path(row["cif_path"]), ref_ca, chain_protein)
    ...
    valid_members["pose_cluster"] = [label_to_cluster.get(l, "noise") for l in labels]
    ...
    for cluster_name in [f"cluster_{i+1}" for i in range(len(order))]:
        ...
        rows.append({
            ...
            "conformation_label": int(rep_row["conformation_label"]) if "conformation_label" in rep_row else -1,
            "rep_cif":           rep_row["cif_path"],
            "rep_model":         rep_row["model"],
            "rep_seed":          int(rep_row["seed"]),
            "rep_sample":        int(rep_row["sample"]),
            "rep_ranking_score": round(float(rep_row["ranking_score"]), 4),
            "rep_iptm_ligand":   round(float(rep_row["iptm_ligand"]), 4),
        })

    summary = pd.DataFrame(rows)
    print(f"    Found {len(summary)} pose clusters")
    print(summary[["pose_cluster", "n", "conformation_label",
                   "rep_model", "rep_seed", "rep_ranking_score"]].to_string(index=False))

    valid_members.to_csv(out_dir / f"{pocket}_members.csv", index=False)
    summary.to_csv(out_dir / f"{pocket}_clusters.csv", index=False)
    write_pymol_script(pocket, summary, ref_cif, out_dir / f"{pocket}_poses.pml")
